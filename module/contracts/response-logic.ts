...
/**
 * Persistent marker for the one model result that has been adopted into the
 * draft. Once set, later refreshes (or other results of the same task) must
 * never silently overwrite the adopted/manual content.
 */
export const responseLogicAppliedModelResultSchema = z
  .object({
    taskId: z.string().trim().min(1).max(255),
    resultId: z.string().trim().min(1).max(512),
    source: z.enum(["structured_output", "assistant_markdown"]),
    contentSha256: z.string().trim().length(64),
    adoptedAt: z.string().datetime(),
  })
  .strict();
export type ResponseLogicAppliedModelResult = z.infer<
  typeof responseLogicAppliedModelResultSchema
>;

/**
 * Last formal publication request. Replaying the same publicationRequestId
 * with identical content returns the recorded success instead of creating a
 * new version; the same id with different content is rejected.
 */
export const responseLogicPublicationMutationSchema = z
  .object({
    publicationRequestId: z.string().uuid(),
    contentSha256: z.string().trim().length(64),
    version: z.number().int().positive(),
  ...
  .strict();
export type ResponseLogicPublicationMutation = z.infer<
  typeof responseLogicPublicationMutationSchema
>;
...
export const saveResponseLogicSchema = responseLogicQuestionSchema.extend({
  ...
  /**
   * Idempotency key for one formal publication attempt. A retried request
   * with the same id and content returns the original success instead of
   * bumping the version again; the same id with different content fails.
   */
  publicationRequestId: z.string().uuid().optional(),
});
...
export type ResponseLogicRecordDto = ResponseLogicQuestion & {
  ...
  /** Set once a model result has been adopted into the draft. */
  appliedModelResult?: {
    taskId: string;
    resultId: string;
