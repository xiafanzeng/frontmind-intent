...
import { z } from "zod";
import { portfolioCandidates } from "../contracts/brand-question-portfolio.js";
import {
...
} from "./brand-question-portfolio-runtime.js";
import type {CredentialHandle,CoreRequest,ResponseLogicApiPorts} from "./task-ports.js";
import type {createBrandQuestionTaskContext} from "./brand-question-task-context.js";
import type {BrandQuestionTaskContextError} from "./brand-question-task-context.js";
import type {ReplaceGeneratedQuestionCandidatesInput} from "./generated-questions.js";
type Port = (...args:any[])=>any;
type TokenService=ReturnType<typeof createBrandQuestionTaskContext>;
export interface QuestionGenerationPorts {
 standaloneWorkspace?():{id:string;ownerUserId:number}|undefined;

 credentialForRequest:ResponseLogicApiPorts["credentialForRequest"];
 createCredentialAgentClient:ResponseLogicApiPorts["createCredentialAgentClient"];
 getCredentialForUpstreamResource:ResponseLogicApiPorts["getCredentialForUpstreamResource"];
 recordUpstreamResource:ResponseLogicApiPorts["recordUpstreamResource"];
 BrandQuestionTaskContextError:new (...args:any[])=>Error & {code:string};
 classifyBrandQuestionTaskStatus:TokenService["classifyBrandQuestionTaskStatus"];
 createBrandQuestionTaskContextToken:TokenService["createBrandQuestionTaskContextToken"];
 verifyBrandQuestionTaskContextToken:TokenService["verifyBrandQuestionTaskContextToken"];
 getDashboardWorkspace:Port;
 getLatestAuthenticatedKnowledgeSnapshot:Port;
 assertServiceCapability:ResponseLogicApiPorts["assertServiceCapability"];
 replaceGeneratedQuestionCandidates:(input:ReplaceGeneratedQuestionCandidatesInput)=>Promise<unknown[]>;
 ServiceEntitlementError:ResponseLogicApiPorts["ServiceEntitlementError"];
 redactSensitivePayload:ResponseLogicApiPorts["redactSensitivePayload"];
 safeErrorForLog:ResponseLogicApiPorts["safeErrorForLog"];
 getUpstreamBaseUrl:ResponseLogicApiPorts["getUpstreamBaseUrl"];
 toUpstreamAgentProfile:ResponseLogicApiPorts["toUpstreamAgentProfile"];
 assertUpstreamPromptBudget:ResponseLogicApiPorts["assertUpstreamPromptBudget"];
 classifyManusV2StructuredResultEnvelope:ResponseLogicApiPorts["classifyManusV2StructuredResultEnvelope"];
 latestManusV2TaskState:ResponseLogicApiPorts["latestManusV2TaskState"];
 ManusV2ApiError:ResponseLogicApiPorts["ManusV2ApiError"];
}
export const manualQuestionInput = z.object({companyName:z.string().trim().min(2).max(200),materials:z.string().trim().min(20).max(60000),questionGoal:z.string().trim().max(2000).default(""),perCategory:z.number().int().min(1).max(10).default(1)}).strict();
export function standaloneQuestionContext(scope:{id:string},modelProfile:BrandQuestionPortfolioContext["modelProfile"],manualInput:unknown):BrandQuestionPortfolioContext {
 const input=manualQuestionInput.parse(manualInput);
 const hash=createHash("sha256").update(JSON.stringify(input)).digest("hex");
 return {modelProfile,planCode:"advanced",quotaPeriodId:scope.id,quotaRevision:1,quota:{industry:input.perCategory,competitorComparison:input.perCategory,reputation:input.perCategory,productScenario:input.perCategory},enterprise:{identityHash:createHash("sha256").update(`${scope.id}\0${input.companyName}`).digest("hex"),canonicalName:input.companyName},snapshot:{id:`manual-${hash.slice(0,24)}`,version:1,archiveHash:hash,sourceFileName:"用户参考材料",documents:[{path:"manual/company-materials.md",title:input.companyName,content:input.materials,kind:"overview"},{path:"manual/question-goal.md",title:"问题目标",content:input.questionGoal||"生成可用于意图优化的问题",kind:"leaf"}]}};
}
export function createQuestionGenerationApi(core:QuestionGenerationPorts) {
const { createCredentialAgentClient, getCredentialForUpstreamResource, recordUpstreamResource, BrandQuestionTaskContextError, classifyBrandQuestionTaskStatus, createBrandQuestionTaskContextToken, verifyBrandQuestionTaskContextToken, getDashboardWorkspace, getLatestAuthenticatedKnowledgeSnapshot, assertServiceCapability, replaceGeneratedQuestionCandidates, ServiceEntitlementError, redactSensitivePayload, safeErrorForLog, getUpstreamBaseUrl, toUpstreamAgentProfile, assertUpstreamPromptBudget, classifyManusV2StructuredResultEnvelope, latestManusV2TaskState, ManusV2ApiError } = core;
...
function sendServiceError(
  ...
  if (error instanceof z.ZodError) {res.status(400).json({error:{code:"INVALID_MANUAL_QUESTION_INPUT",message:"请检查企业名称、参考材料和生成数量。"}});return;}
...
function publicBrandQuestionTask(
...
function brandQuestionTaskContextErrorResponse(
...
async function currentContext(
 ...
 const standaloneScope=core.standaloneWorkspace?.();
 if(standaloneScope)return {context:standaloneQuestionContext(standaloneScope,modelProfile,manualInput)};
...
function createBrandQuestionTaskIdempotencyKey(input: {
  userId: number;
  prompt: string;
...
function createBrandQuestionFileIdempotencyKey(input: {
...
async function createBrandQuestionUpstreamTask(input: {
...
router.post("/start", async (request, res) => {
 ...
 const requestCredential=core.credentialForRequest(request);
  try {
    ...
    const { context } = await currentContext(
      user.id,
      requestCredential.agentProfile,
      req.body?.manualInput,
    );
    const baseUrl = getUpstreamBaseUrl(req);
    const apiKey = requestCredential.credentialRef;
    ...
    const client = createCredentialAgentClient(requestCredential, {
            ...
            observer: {
              onCandidateCreated: async ({ fileId }: {fileId:string}) => {
                await recordUpstreamResource({
                  userId: user.id,
                  apiCredentialId: requestCredential!.id,
                  kind: "file",
                  upstreamId: fileId,
                });
              },
    ...
    const contextToken = createBrandQuestionTaskContextToken({
      ...
      secret: requestCredential.credentialRef,
    });
    res.json({
      task: publicBrandQuestionTask(
        { status: "running" },
        taskId,
        requestCredential.credentialRef,
      ),
      contextToken,
      startedAt: Date.now(),
      ...
      quotaPeriodId: context.quotaPeriodId,
      model: requestCredential.agentProfile,
    });
  } catch (error) {
    sendServiceError(res, error, requestCredential?.credentialRef);
  }
});
...
router.post("/sync", async (request, res) => {
 ...
 const requestCredential=core.credentialForRequest(request);
  ...
  let logSecret = requestCredential?.credentialRef;
  try {
    const user = req.frontmindUser;
    if (!user) {
      res.status(401).json({
    ...
    const { taskId, contextToken, manualInput } = z
      .object({
        taskId: z.string().trim().min(1).max(255),
        contextToken: z.string().trim().min(1).max(4_096),
        manualInput: manualQuestionInput.optional(),
      })
      .strict()
      .parse(req.body || {});
    ...
    const { context } = await currentContext(user.id, credential.agentProfile, manualInput);
    logSecret = credential.credentialRef;
    verifyBrandQuestionTaskContextToken({
      token: contextToken,
      secret: credential.credentialRef,
      expected: {
        userId: user.id,
        taskId,
        snapshotId: context.snapshot.id,
...
const result={publicBrandQuestionTask, brandQuestionTaskContextErrorResponse, createBrandQuestionTaskIdempotencyKey, createBrandQuestionFileIdempotencyKey, createBrandQuestionUpstreamTask};
