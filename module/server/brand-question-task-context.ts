...
const servicePlanCodeSchema = z.enum(["basic", "advanced", "luxury"]);
...
export function createBrandQuestionTaskContext(core:{sign(payload:string,credentialRef:string):Buffer}) {
function signature(payload: string, secret: string) {
  ...
  return core.sign(payload, secret);
}
...
function verifyBrandQuestionTaskContextToken(input: {
  token: string;
  secret: string;
  now?: Date;
  expected: {
...
function classifyBrandQuestionTaskStatus(value: unknown) {
...
return { createBrandQuestionTaskContextToken, verifyBrandQuestionTaskContextToken, classifyBrandQuestionTaskStatus };
}
