...
import type { CoreProviderError, CoreRequest, CredentialHandle, ManusV2MessageEvent, ResponseLogicApiPorts } from "./task-ports.js";
/** Business routes, evidence assembly, task binding, adoption and recovery live in the module.
 * The host only supplies identity, transactional repositories and generic execution/file services. */
export function createResponseLogicApi(core: ResponseLogicApiPorts) {
 const { projectGeneralExecution, generalExecutionActivity, orderExecutionTimeline, projectBusinessExecution, enterpriseWorkspaceUserId, getEnterpriseProjectScope, enterpriseAccountOwnerPredicate, sendAiBillingError, localAssets, providerFileLeases, getCredentialForUpstreamResource, recordUpstreamResource, createCredentialAgentClient, fileResourceContentExpiry, isFileResourceContentExpired, getDashboardQuestion, getLatestKnowledgeSnapshot, getFrontMindCredentials, getUpstreamBaseUrl, toUpstreamAgentProfile, ResponseLogicConfirmedError, ResponseLogicProviderReadinessError, ResponseLogicPublicationRequestConflictError, ResponseLogicResultAdoptionError, ResponseLogicRevisionConflictError, ResponseLogicTaskActiveError, ResponseLogicTaskSupersededError, adoptResponseLogicModelResult, assertResponseLogicRecordEditable, getResponseLogicEntry, recordResponseLogicTaskStart, releaseResponseLogicTaskBinding, requireResponseLogicProviderReadiness, updateResponseLogicDraftMaterials, assertServiceCapability, ServiceEntitlementError, redactSensitivePayload, redactSensitiveText, safeErrorForLog, buildDeterministicTaskAttachmentArchive, buildDirectorySkillArchive, assertUpstreamPromptBudget, getDb, readStoredPresalesFile, classifyManusV2StructuredResultEnvelope, latestManusV2TaskState, ManusV2ApiError, manusV2EventOperationToken, orderManusV2EventsByProviderRank } = core;
const router: import("express").Router = Router();

function responseLogicPublicExecution(taskId: string, events: readonly ManusV2MessageEvent[]) {
  let turn = 0;
...
const RESPONSE_LOGIC_STRUCTURED_OUTPUT_SCHEMA = {
  type: "object",
  properties: {
    concern: { type: "string" },
...
function responseLogicStructuredDraftFromV2Events(
...
function validatedPublicResponseLogicStructuredDraft(
  value: unknown,
 ...
 */
function currentResponseLogicRoundEvents(
  events: ReadonlyArray<ManusV2MessageEvent>,
) {
 ...
 */
function responseLogicTaskResultFromCurrentV2Round(
  events: ReadonlyArray<ManusV2MessageEvent>,
...
function responseLogicTaskStatusEnvelopeRoundTrip(
...
const RESPONSE_LOGIC_UPSTREAM_ATTACHMENT_LIMIT = 102;
const RESPONSE_LOGIC_CUSTOMER_ATTACHMENT_LIMIT = 99;
...
const responseLogicAdoptResultSchema = z
  .object({
    questionId: z.string().trim().min(1).max(191),
    conversationId: z.string().trim().min(1).max(191),
...
const localAssetIdSchema = z
  .string()
  .trim()
  .regex(
    /^(?:asset_)?[a-f0-9]{30,32}$/u,
    "材料标识格式无效",
  );

const responseLogicMaterialsSchema = z
  .object({
    questionId: z.string().trim().min(1).max(191),
    expectedRevision: z.number().int().nonnegative(),
    addAssetIds: z.array(localAssetIdSchema).max(50).default([]),
    removeAssetIds: z.array(localAssetIdSchema).max(50).default([]),
  })
  .strict();

/** Local asset row ids keep their asset_ prefix; requests may echo the id
 * exactly or send the bare hex tail. Only the bare form is re-prefixed. */
function normalizeLocalAssetId(value: string) {
  return /^[a-f0-9]{30,32}$/u.test(value) ? `asset_${value}` : value;
}

class ResponseLogicTaskBindingError extends Error {
  constructor(
...
type ResponseLogicStartFailureStage =
  | "file_upload_intent"
  | "file_upload_content"
  | "file_confirmation"
  | "task_create"
...
type ResponseLogicStartFailureEnvelope = {
  code:
...
function responseLogicPostDispatchBindingFailure(
...
function responseLogicStartFailureFromManusError(input: {
  error: CoreProviderError;
  incidentId?: string;
...
function responseLogicRecordMatchesConfiguredQuestion(input: {
  record: Pick<
 ...
 */
function assertResponseLogicTaskBinding(input: {
...
type NormalizedResponseLogicTaskStatus =
  | "running"
  | "completed"
  | "failed"
  | "unknown";

function normalizeResponseLogicTaskStatus(
  status: unknown,
 ...
 */
function buildVerifiedResponseLogicAttachments(
...
const skillDirectoryCandidates = configuredResponseLogicSkillPath
  ? [configuredResponseLogicSkillPath]
  : [path.resolve(import.meta.dirname, "../workflows/response-logic-builder.skill"), path.resolve(process.cwd(), "modules/intent/workflows/response-logic-builder.skill"), path.resolve(process.cwd(), "private-workflows/response-logic-builder.skill")];
...
const RESPONSE_LOGIC_SKILL_ATTACHMENT_FILENAME =
  "response-logic-builder.skill.zip";
const RESPONSE_LOGIC_EVIDENCE_ATTACHMENT_FILENAME =
  "response-logic-evidence.zip";
const RESPONSE_LOGIC_TURN_INPUT_ATTACHMENT_FILENAME =
...
function responseLogicEvidenceAttachmentFilename(contentHash: string) {
...
function responseLogicTurnInputAttachmentFilename(contentHash: string) {
  return contentAddressedAttachmentFilename(
...
function createResponseLogicTaskIdempotencyKey(input: {
  userId: number;
  conversationId: string;
  questionId: string;
  taskId?: string;
...
function createResponseLogicFileIdempotencyKey(input: {
...
function assertResponseLogicAttachmentCapacity(input: {
...
async function buildResponseLogicEvidenceArchive(
...
async function buildResponseLogicTurnInputArchive(input: {
...
function publicResponseLogicTask(
  payload: unknown,
  taskId: string,
  apiKey: string,
) {
...
const RESPONSE_LOGIC_TASK_STATUS_CACHE_CONTROL =
  "private, no-store, max-age=0";

function setResponseLogicTaskStatusNoStore(input: {
  setHeader: (name: string, value: string) => unknown;
}) {
  input.setHeader("Cache-Control", RESPONSE_LOGIC_TASK_STATUS_CACHE_CONTROL);
}

router.get("/tasks/:taskId/status", async (request, res) => {
  const req = request as CoreRequest;
  const requestCredential = core.credentialForRequest(request);
  ...
  try {
    ...
    logSecret = credential.credentialRef;
...
router.post("/adopt-result", async (request, res) => {
  const req = request as CoreRequest;
  const requestCredential = core.credentialForRequest(request);
  const parsed = responseLogicAdoptResultSchema.safeParse(req.body || {});
  if (!parsed.success) {
    res.status(400).json({
      error: {
        code: "INVALID_RESPONSE_LOGIC_ADOPT_INPUT",
        message: "缺少当前问题、会话、任务或结果标识",
      },
    });
    return;
  }
  ...
  try {
    ...
    const [record, credential] = await Promise.all([
      getResponseLogicEntry(workspaceUserId, parsed.data.questionId),
    ...
    assertResponseLogicTaskBinding({
      ...
      operationRevision: parsed.data.operationRevision,
      record,
      configuredQuestion,
    });
    if (!credential) {
    ...
    // The adopted content always comes from the provider record on file, never
    ...
    if (status === null || status === "running" || status === "waiting") {
      res.status(409).json({
        error: {
          code: "RESPONSE_LOGIC_TASK_RUNNING",
          message: "应答逻辑任务尚未完成，请等待生成结束后再采用结果",
        },
      });
      return;
    }
    ...
    if (!result) {
      res.status(422).json({
        error: {
          code: "RESPONSE_LOGIC_TASK_OUTPUT_INVALID",
          message: "模型输出未通过四栏目校验，不能采用该结果",
        },
      });
      return;
    }
    if (result.resultId !== parsed.data.resultId) {
      res.status(409).json({
        error: {
          code: "RESPONSE_LOGIC_RESULT_SUPERSEDED",
          message: "任务已产生新的结果，请采用最新结果",
          resultId: result.resultId,
        },
      });
      return;
    }
    const outcome = await adoptResponseLogicModelResult({
      userId: workspaceUserId,
      questionId: parsed.data.questionId,
      conversationId: parsed.data.conversationId,
      taskId: parsed.data.taskId,
      ...
      source: result.source,
      structuredDraft: result.structuredDraft,
      expectedOperationRevision: parsed.data.operationRevision,
    });
    res.json({
      status: "adopted",
    ...
    if (
      ...
      error instanceof ResponseLogicResultAdoptionError ||
      error instanceof ResponseLogicPublicationRequestConflictError
    ) {
      res.status(error.statusCode).json({
        error: { code: error.responseLogicCode, message: error.message },
      });
    ...
    console.error(
      "[Response Logic Adopt] error:",
      safeErrorForLog(error, { secrets: [logSecret] }),
    );
    res.status(500).json({
      error: {
        code: "RESPONSE_LOGIC_ADOPT_FAILED",
        message: "采用应答逻辑结果失败，请稍后重试",
      },
    });
  }
});

router.post("/materials", async (request, res) => {
  const req = request as CoreRequest;
  const requestCredential = core.credentialForRequest(request);
  const parsed = responseLogicMaterialsSchema.safeParse(req.body || {});
  if (!parsed.success) {
    res.status(400).json({
      error: {
        code: "INVALID_RESPONSE_LOGIC_MATERIALS_INPUT",
        message: "材料变更请求格式不完整",
      },
    });
    return;
  ...
  try {
    ...
    const record = await getResponseLogicEntry(
      workspaceUserId,
      parsed.data.questionId,
    );
    if (!record) {
      res.status(404).json({
        error: {
          code: "RESPONSE_LOGIC_RECORD_MISSING",
          message: "当前问题还没有应答逻辑内容，请先生成或保存草稿",
        },
      });
      return;
    }
    const addAssetIds = [
      ...new Set(parsed.data.addAssetIds.map(normalizeLocalAssetId)),
    ];
    const removeFileIds = [
      ...new Set(parsed.data.removeAssetIds.map(normalizeLocalAssetId)),
    ];
    if (addAssetIds.length === 0 && removeFileIds.length === 0) {
      res.status(400).json({
        error: {
          code: "RESPONSE_LOGIC_MATERIALS_NO_CHANGE",
          message: "没有需要变更的材料",
        },
      });
      return;
    ...
    if (!db) {
      res.status(503).json({
        error: {
          ...
          message: "材料存储暂时不可用，请稍后重试",
        },
      });
      return;
    }
    const rows = (addAssetIds.length > 0
        ? await db
            .select()
            .from(localAssets)
            .where(
              and(
                inArray(localAssets.id, addAssetIds),
                eq(localAssets.scope, "managed_user"),
                enterpriseAccountOwnerPredicate(localAssets, workspaceUserId),
              ),
            )
        : []) as Array<{id:string;sizeBytes:number;contentSha256:string;filename:string;mimeType:string;createdAt:Date;contentExpiresAt:Date|null;contentDeletedAt:Date|null}>;
    const rowById = new Map(rows.map((row) => [row.id, row]));
    for (const assetId of addAssetIds) {
      const row = rowById.get(assetId);
      const stored = row ? await readStoredPresalesFile(assetId) : null;
      if (
        !row ||
        !stored ||
        ...
        res.status(403).json({
          error: {
            ...
            message: "材料不存在、不属于当前账号或校验失败，请重新上传",
          },
        });
        return;
      }
    }
    // Names, types and timestamps come from the asset records themselves.
    const lifecycleByFileId = new Map(
      rows.map((row) => [row.id, { uploadedAt: row.createdAt }]),
    );
    const addAttachments = buildVerifiedResponseLogicAttachments(
      addAssetIds.map((assetId) => ({
        file_id: assetId,
        filename: rowById.get(assetId)!.filename,
        mime_type: rowById.get(assetId)!.mimeType,
      })),
      lifecycleByFileId,
    );
    const keptFileIds = new Set(
      record.draft.attachments
        .filter((attachment) => !removeFileIds.includes(attachment.fileId))
        .map((attachment) => attachment.fileId),
    );
    const mergedCount = new Set([
      ...keptFileIds,
      ...addAttachments.map((attachment) => attachment.fileId),
    ]).size;
    if (mergedCount > RESPONSE_LOGIC_CUSTOMER_ATTACHMENT_LIMIT) {
      res.status(400).json({
        error: {
          code: "RESPONSE_LOGIC_MATERIALS_LIMIT",
          message: `单个问题最多绑定 ${RESPONSE_LOGIC_CUSTOMER_ATTACHMENT_LIMIT} 份材料`,
        },
      });
      return;
    }

    const saved = await updateResponseLogicDraftMaterials({
      userId: workspaceUserId,
      questionId: parsed.data.questionId,
    ...
    res.json({ status: "updated", record: saved });
  } catch (error) {
    if (sendAiBillingError(res, error)) return;
    if (error instanceof ServiceEntitlementError) {
      res.status(error.statusCode).json({
    ...
    console.error("[Response Logic Materials] error:", safeErrorForLog(error));
    res.status(500).json({
      error: {
        code: "RESPONSE_LOGIC_MATERIALS_FAILED",
        message: "更新应答逻辑材料失败，请稍后重试",
      },
    });
  }
});

router.post(["/start", "/turn"], async (request, res) => {
  const req = request as CoreRequest;
  const requestCredential = core.credentialForRequest(request);
  ...
  if (!req.frontmindUser || !requestCredential) {
  ...
  if (!activeCredentials.credentialRef) {
    res.status(401).json({ error: { message: "尚未配置 API Key" } });
    return;
  }

  let logSecret = activeCredentials.credentialRef;
  try {
    const configuredQuestion = await getDashboardQuestion(
      workspaceUserId,
      parsed.data.questionId,
    ...
    let taskApiKey = activeCredentials.credentialRef;
    let taskCredential = requestCredential;
    if (value.taskId) {
      ...
      taskApiKey = boundTaskCredential.credentialRef;
    ...
    for (const attachmentPackage of generatedAttachmentPackages) {
      const uploaded = await responseLogicClient.uploadFile({
        ...
        observer: {
          onCandidateCreated: async ({ fileId }: {fileId:string}) => {
...
const result = { responseLogicPublicExecution, RESPONSE_LOGIC_STRUCTURED_OUTPUT_SCHEMA, responseLogicStructuredDraftFromV2Events, validatedPublicResponseLogicStructuredDraft, currentResponseLogicRoundEvents, responseLogicTaskResultFromCurrentV2Round, responseLogicTaskStatusEnvelopeRoundTrip, RESPONSE_LOGIC_UPSTREAM_ATTACHMENT_LIMIT, RESPONSE_LOGIC_CUSTOMER_ATTACHMENT_LIMIT, ResponseLogicTaskBindingError, responseLogicPostDispatchBindingFailure, responseLogicStartFailureFromManusError, responseLogicRecordMatchesConfiguredQuestion, assertResponseLogicTaskBinding, normalizeResponseLogicTaskStatus, buildVerifiedResponseLogicAttachments, RESPONSE_LOGIC_SKILL_ATTACHMENT_FILENAME, RESPONSE_LOGIC_EVIDENCE_ATTACHMENT_FILENAME, RESPONSE_LOGIC_TURN_INPUT_ATTACHMENT_FILENAME, responseLogicEvidenceAttachmentFilename, responseLogicTurnInputAttachmentFilename, createResponseLogicTaskIdempotencyKey, createResponseLogicFileIdempotencyKey, assertResponseLogicAttachmentCapacity, buildResponseLogicSkillArchive, getResponseLogicSkillDescriptor, buildResponseLogicEvidenceArchive, buildResponseLogicTurnInputArchive, buildResponseLogicPrompt, createResponseLogicTask, publicResponseLogicTask, RESPONSE_LOGIC_TASK_STATUS_CACHE_CONTROL, setResponseLogicTaskStatusNoStore };
return Object.assign(result, { router: router as import("express").Router });
}
